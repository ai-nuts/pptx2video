#!/usr/bin/env python3
"""Quality gate for a pptx2video package.
...
from .audio_files import load_audio_manifest_file_map, resolve_audio_path
...
ANIMATION_RENDER_STRATEGIES = {
    ...
    "Split In": "split_center_approx",
    "Blinds In": "blinds_vertical_approx",
    "Checkerboard In": "checkerboard_tiles_approx",
    "Dissolve In": "dissolve_alpha_approx",
    "Random Bars In": "random_bars_deterministic_approx",
    "Peek In": "peek_from_left_approx",
    "Wheel In": "wheel_circle_approx",
    "Box In": "box_reveal",
    "Circle In": "circle_reveal",
    "Diamond In": "diamond_reveal",
    "Plus In": "plus_reveal",
    "Strips In": "diagonal_strips_approx",
    "Wedge In": "diagonal_wedge_approx",
    "Stretch In": "stretch_horizontal",
    "Expand In": "expand_from_center",
    "Swivel In": "swivel_horizontal_approx",
}
...
def find_ffmpeg_pair() -> tuple[str | None, str | None]:
    return _shared_ffmpeg_pair(required=False)
...
def check_audio(audio_dir: Path | None, sections: list[dict[str, str]], ffprobe: str | None, ffmpeg: str | None, findings: list[Finding]) -> dict[str, Any]:
    ...
    try:
        ...
        add(findings, "error", "audio_manifest_invalid", str(exc), location=str(audio_dir / "manifest.json"))
        return {"checked": True, "files": [], "total_duration": 0.0}
    for sec in sections:
        try:
            path = resolve_audio_path(audio_dir, sec["id"], file_map=audio_file_map)
        except ValueError as exc:
            add(findings, "error", "audio_mapping_invalid", str(exc), location=str(audio_dir), section_id=sec["id"])
            continue
        referenced_filenames.add(path.name)
        if not path.is_file():
    ...
    extra = sorted(p.name for p in audio_dir.glob("*.mp3") if p.name not in referenced_filenames)
    if extra:
...
def _check_pptx_sequence_schedule(
    ...
    *,
    ...
    """Validate serialized Animate -> Speak playback for editable PPTX slides."""
    checked_blocks = 0
    checked_effects = 0
    gate_violations = 0
    expected_policy = "narrated_block_serial_v1"
    native_resolutions = {
        "native_pane_phase_in_block_preroll",
        "native_pane_phase_in_word_timing_gap",
        "native_pane_phase_in_section_postroll",
        "native_pane_authority",
    }
    for slide in manifest.get("slides") or []:
        slide_index = int(slide.get("index") or 0)
        if slide.get("schedule_policy") != expected_policy:
            add(findings, "error", "animation_sequence_policy_missing", "Editable PPTX animation manifest is missing the narrated-block serialized schedule policy.", location=str(path), slide_index=slide_index)
            continue
        blocks = slide.get("sequence_blocks") or []
        blocks_by_index: dict[int, dict[str, Any]] = {}
        native_effects: list[tuple[int, dict[str, Any]]] = []
        expected_schedule_end = 0.0
        for expected_index, block in enumerate(blocks, start=1):
            ...
            if release_before > spoken_end + 0.002 or abs(release - spoken_end) > 0.002:
                add(findings, "error", "animation_narration_window_bad", "Notes narration block has an invalid word-timing window.", location=str(path), slide_index=slide_index, block_index=block_index, release_before=release_before, spoken_end=spoken_end, release=release)
            narrated = bool(block.get("narrated"))
            if narrated:
                try:
                    narration_start = float(block.get("narration_start"))
                    pre_roll_seconds = float(block.get("pre_roll_seconds"))
                    pre_roll_start = float(block.get("pre_roll_start"))
                    pre_roll_end = float(block.get("pre_roll_end"))
                except (TypeError, ValueError):
                    add(findings, "error", "animation_preroll_missing", "A narrated block is missing output-audio pre-roll evidence.", location=str(path), slide_index=slide_index, block_index=block_index, block=block)
                else:
                    if (
                        pre_roll_seconds < -0.002
                        or pre_roll_end + 0.002 < pre_roll_start
                        or abs((pre_roll_end - pre_roll_start) - pre_roll_seconds) > 0.003
                        or pre_roll_end > narration_start + 0.002
                    ):
                        add(findings, "error", "animation_preroll_window_bad", "Narrated block pre-roll is not a valid Animate-before-Speak window.", location=str(path), slide_index=slide_index, block_index=block_index, pre_roll_seconds=pre_roll_seconds, pre_roll_start=pre_roll_start, pre_roll_end=pre_roll_end, narration_start=narration_start)
            expected_schedule_end = max(expected_schedule_end, release)
            for effect in block.get("effects") or []:
                ...
                timing_resolution = str(effect.get("timing_resolution") or "")
                if timing_source == "animation_pane":
                    native_effects.append((block_index, effect))
                    if timing_resolution not in native_resolutions:
                        add(findings, "error", "animation_pane_authority_missing", "Native animation timing is not marked as a supported serialized Pane phase.", location=str(path), slide_index=slide_index, block_index=block_index, effect=effect)
                    if gate < -0.002 or start + 0.002 < gate:
                        gate_violations += 1
                        add(findings, "error", "animation_sequence_gate_violated", "A native effect begins before its preceding narrated-block release gate.", location=str(path), slide_index=slide_index, block_index=block_index, handle=block.get("handle"), gate=gate, start=start)
                elif timing_source == "edge_word_alignment":
                    if timing_resolution != "supplemental_spotlight_marker":
                        add(findings, "error", "animation_spotlight_timing_bad", "Supplemental Spotlight is not aligned to an Edge word boundary.", location=str(path), slide_index=slide_index, block_index=block_index, effect=effect)
                else:
                    add(findings, "error", "animation_timing_source_bad", "Animation effect has an unknown timing authority.", location=str(path), slide_index=slide_index, block_index=block_index, effect=effect)
                expected_schedule_end = max(expected_schedule_end, end)

        phases: dict[int, dict[str, Any]] = {}
        for owner_index, effect in native_effects:
            try:
                phase_index = int(effect.get("phase_index"))
                start = float(effect.get("start"))
                end = float(effect.get("end"))
                gate = float(effect.get("sequence_gate"))
                targets = [int(value) for value in effect.get("phase_target_block_indices") or []]
            except (TypeError, ValueError):
                add(findings, "error", "animation_phase_bad", "Serialized native effect has invalid phase metadata.", location=str(path), slide_index=slide_index, block_index=owner_index, effect=effect)
                continue
            phase = phases.setdefault(
                phase_index,
                {"starts": [], "ends": [], "gates": [], "targets": set(), "owners": set()},
            )
            phase["starts"].append(start)
            phase["ends"].append(end)
            phase["gates"].append(gate)
            phase["targets"].update(targets)
            phase["owners"].add(owner_index)

        narrated_indices = [
            index for index, block in blocks_by_index.items() if bool(block.get("narrated"))
        ]
        target_phase_by_block: dict[int, int] = {}
        for phase_index in sorted(phases):
            phase = phases[phase_index]
            targets = sorted(phase["targets"])
            if not targets:
                # Native-only slides and trailing post-roll legitimately have no
                # following narrated target.
                continue
            target_blocks = [blocks_by_index.get(index) for index in targets]
            if any(block is None or not bool(block.get("narrated")) for block in target_blocks):
                add(findings, "error", "animation_phase_target_bad", "Serialized animation phase targets a missing or silent block.", location=str(path), slide_index=slide_index, phase_index=phase_index, targets=targets)
                continue
            for target in targets:
                target_phase_by_block[target] = phase_index
            previous_phase_spoken_end = max(
                [
                    float(blocks_by_index[index]["spoken_end"])
                    for index in narrated_indices
                    if index < min(targets)
                ]
                + [0.0]
            )
            target_narration_start = min(float(block["narration_start"]) for block in target_blocks if block is not None)
            phase_start = min(phase["starts"])
            phase_end = max(phase["ends"])
            if phase_start + 0.002 < previous_phase_spoken_end:
                gate_violations += 1
                add(findings, "error", "animation_before_prior_narration_end", "The next animation phase starts before the preceding narrated block finishes.", location=str(path), slide_index=slide_index, phase_index=phase_index, phase_start=phase_start, prior_spoken_end=previous_phase_spoken_end)
            if any(abs(float(gate) - previous_phase_spoken_end) > 0.003 for gate in phase["gates"]):
                gate_violations += 1
                add(findings, "error", "animation_phase_gate_mismatch", "Serialized animation phase does not use the preceding narrated-block end as its gate.", location=str(path), slide_index=slide_index, phase_index=phase_index, expected_gate=previous_phase_spoken_end, actual_gates=phase["gates"])
            if phase_end > target_narration_start + 0.002:
                gate_violations += 1
                add(findings, "error", "animation_overlaps_narration", "A narrated entrance or silent connector continues after the target narration begins.", location=str(path), slide_index=slide_index, phase_index=phase_index, phase_end=phase_end, narration_start=target_narration_start, targets=targets)
        previous_narrated_index: int | None = None
        for block_index in narrated_indices:
            block = blocks_by_index[block_index]
            own_native = [
                effect
                for owner, effect in native_effects
                if owner == block_index and str(effect.get("kind") or "entrance") == "entrance"
            ]
            narration_start = float(block.get("narration_start") or 0.0)
            if own_native and max(float(effect["end"]) for effect in own_native) > narration_start + 0.002:
                gate_violations += 1
                add(findings, "error", "narrated_entrance_after_script_start", "A narrated element entrance has not finished before its script starts.", location=str(path), slide_index=slide_index, block_index=block_index, narration_start=narration_start)
            if previous_narrated_index is not None:
                current_phase = target_phase_by_block.get(block_index)
                previous_phase = target_phase_by_block.get(previous_narrated_index)
                if current_phase is not None and current_phase != previous_phase:
                    phase_start = min(phases[current_phase]["starts"])
                    prior_spoken_end = float(blocks_by_index[previous_narrated_index]["spoken_end"])
                    if phase_start + 0.002 < prior_spoken_end:
                        gate_violations += 1
                        add(findings, "error", "next_entrance_before_prior_script_end", "The next element animation starts before the prior element script ends.", location=str(path), slide_index=slide_index, block_index=block_index, phase_start=phase_start, prior_spoken_end=prior_spoken_end)
            previous_narrated_index = block_index
        try:
            ...
            if abs(schedule_end - expected_schedule_end) > 0.002:
                add(findings, "error", "animation_sequence_end_mismatch", "Animation schedule end differs from the latest narration or effect end.", location=str(path), slide_index=slide_index, expected=expected_schedule_end, actual=schedule_end)
    return {
        "schedule_policy": expected_policy,
    ...
    *,
    ...
    for key, expected in manifest_effects.items():
        ...
        try:
            ...
            continue
        if expected_strategy in {"appear", "cut_instant"}:
...
def check_animation_order_authority(
    path: Path | None,
    *,
    required: bool,
    findings: list[Finding],
    ...
    """Require an explicit, resolved narration-order versus Pane decision."""
    if path is None:
        if required:
            add(
                findings,
                "error",
                "animation_order_authority_required",
                "Strict editable-PPTX QA requires --animation-order-authority.",
            )
        return {"checked": False}
    if not path.is_file():
        add(
            findings,
            "error",
            "animation_order_authority_missing",
            "Animation order authority report is missing.",
            location=str(path),
        )
        return {"checked": False}
    payload = read_json(path)
    if not isinstance(payload, dict) or payload.get("schema_version") != (
        "paper2video_animation_order_authority.v1"
    ):
        add(
            findings,
            "error",
            "animation_order_authority_schema",
            "Animation order authority report has an unsupported schema.",
            location=str(path),
        )
        return {"checked": True}
    status = str(payload.get("status") or "")
    policy = str(payload.get("selected_policy") or "")
    try:
        conflict_count = int(payload.get("conflict_count") or 0)
        inversion_pair_count = int(payload.get("inversion_pair_count") or 0)
    except (TypeError, ValueError):
        add(
            findings,
            "error",
            "animation_order_count_invalid",
            "Animation order conflict counts must be integers.",
            location=str(path),
        )
        conflict_count = 0
        inversion_pair_count = 0

    report_slides = payload.get("slides")
    if not isinstance(report_slides, list):
        add(
            findings,
            "error",
            "animation_order_slides_invalid",
            "Animation order authority slides must be a list.",
            location=str(path),
        )
        report_slides = []
    reported_conflicts = [
    ...
    reported_pairs: set[tuple[int, str, str]] = set()
    ...
    for slide in report_slides:
        ...
        try:
            ...
            add(
                findings,
                "error",
                "animation_order_rank_invalid",
                "Animation order slide indices and ranks must be integers.",
                location=str(path),
            )
            slide_index = 0
        for conflict in slide.get("conflicts") or []:
            if not isinstance(conflict, dict):
                continue
            shape_id = str(conflict.get("shape_id") or "")
            inversions = [
                inversion
                for inversion in conflict.get("inversions") or []
                if isinstance(inversion, dict)
            ]
            try:
                declared_inversion_count = int(
                    conflict.get("inversion_count") or 0
                )
                reading_rank = int(conflict.get("reading_rank") or 0)
                pane_rank = int(conflict.get("pane_rank") or 0)
            except (TypeError, ValueError):
                add(
                    findings,
                    "error",
                ...
                declared_inversion_count = 0
                reading_rank = 0
                pane_rank = 0
            if declared_inversion_count != len(inversions):
                add(
                    findings,
                    "error",
                    "animation_order_inversion_count_mismatch",
                    "A conflict inversion_count does not match its inversion rows.",
                    location=str(path),
                    shape_id=shape_id,
                )
            inversion_shape_ids = [
                str(inversion.get("shape_id") or "") for inversion in inversions
            ]
            for anchor_shape_id in inversion_shape_ids:
                reported_pairs.add(
                    (slide_index, *sorted((shape_id, anchor_shape_id)))
                )
            fingerprint_rows.append(
                {
                    ...
                    "pane_rank": pane_rank,
                    "inversions": inversion_shape_ids,
                }
            )
    if conflict_count != len(reported_conflicts):
        add(
            findings,
            "error",
            "animation_order_conflict_count_mismatch",
            "Animation order conflict_count does not match the reported conflicts.",
            location=str(path),
            declared=conflict_count,
            actual=len(reported_conflicts),
        )
    if inversion_pair_count != len(reported_pairs):
        add(
            findings,
            "error",
            "animation_order_pair_count_mismatch",
            "Animation order inversion_pair_count does not match unique pairs.",
            location=str(path),
            declared=inversion_pair_count,
            actual=len(reported_pairs),
        )
    if status == "no_conflict" and conflict_count != 0:
        add(
            findings,
            "error",
            "animation_order_status_inconsistent",
            "A no_conflict report cannot contain order conflicts.",
            location=str(path),
        )
    if status == "resolved" and conflict_count <= 0:
        add(
            findings,
            "error",
            "animation_order_status_inconsistent",
            "A resolved report must identify at least one confirmed conflict.",
            location=str(path),
        )
    if status not in {"no_conflict", "resolved"}:
        add(
            findings,
            "error",
            "animation_order_decision_unresolved",
            "Animation order conflict was not resolved before rendering.",
            location=str(path),
            status=status,
        )
    if policy != "animation-pane":
        add(
            findings,
            "error",
            "animation_order_policy_unsupported",
            "This renderer requires a confirmed Animation Pane order.",
            location=str(path),
            selected_policy=policy,
        )
    if conflict_count and not payload.get("selection_source"):
        add(
            findings,
            "error",
            "animation_order_selection_source_missing",
            "Resolved animation order conflicts must record how the user selected the policy.",
            location=str(path),
        )
    hash_pattern = re.compile(r"^[0-9a-f]{64}$")
    for field_name in ("conflict_set_fingerprint", "conflict_fingerprint"):
        if not hash_pattern.fullmatch(str(payload.get(field_name) or "")):
            add(
                findings,
                "error",
                "animation_order_fingerprint_invalid",
                f"Animation order {field_name} must be a SHA-256 hex digest.",
                location=str(path),
                field=field_name,
            )
    expected_set_fingerprint = hashlib.sha256(
        json.dumps(
        ...
        json.dumps(
            {
                ...
                "conflict_set_fingerprint": expected_set_fingerprint,
            },
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
    ...
    if payload.get("conflict_set_fingerprint") != expected_set_fingerprint or (
        payload.get("conflict_fingerprint") != expected_source_fingerprint
    ):
        add(
            findings,
            "error",
            "animation_order_fingerprint_mismatch",
            "Animation order fingerprints do not match the reported conflict set.",
            location=str(path),
        )
    if conflict_count:
        confirmed_fingerprint = str(
            payload.get("confirmed_conflict_fingerprint") or ""
        )
        if confirmed_fingerprint != str(payload.get("conflict_fingerprint") or ""):
            add(
                findings,
                "error",
                "animation_order_confirmation_stale",
                "The confirmed conflict fingerprint does not match the rendered report.",
                location=str(path),
            )
    if pptx_path is not None:
        expected_hash = str(payload.get("normalized_source_sha256") or "")
        actual_hash = sha256_file(pptx_path) if pptx_path.is_file() else ""
        if not expected_hash or expected_hash != actual_hash:
            add(
                findings,
                "error",
                "animation_order_pptx_hash_mismatch",
                "Animation order authority is not bound to the delivered PPTX bytes.",
                location=str(pptx_path),
    ...
    return {
        ...
        "inversion_pair_count": inversion_pair_count,
        "conflict_set_fingerprint": payload.get("conflict_set_fingerprint"),
...
def maybe_write_manifest(project_dir: Path, args: argparse.Namespace, report_path: Path, passed: bool) -> None:
    ...
    if args.animation_order_authority:
        files["animation_order_authority"] = rel_to(
            args.animation_order_authority.resolve(), project_dir
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run deterministic QA gates for a pptx2video package.")
    ...
    parser.add_argument("--animation-order-authority", type=Path, help="Resolved narration-order versus Animation Pane decision report.")
    ...
    parser.add_argument("--require-animation-order-authority", action="store_true", help="Fail unless animation order conflicts are absent or explicitly resolved.")
    ...
    animation_order_authority_report = check_animation_order_authority(
        args.animation_order_authority.resolve()
        if args.animation_order_authority
        else None,
        required=args.require_animation_order_authority,
        findings=findings,
        pptx_path=args.pptx.resolve() if args.pptx else None,
    )
    ...
    report = {
        ...
        "inputs": {
            ...
            "animation_order_authority": str(args.animation_order_authority.resolve()) if args.animation_order_authority else None,
        },
        "options": {
            "strict": args.strict,
            ...
            "require_animation_order_authority": args.require_animation_order_authority,
            "strict_attention": strict_attention_required,
        ...
        "animation_order_authority": animation_order_authority_report,
