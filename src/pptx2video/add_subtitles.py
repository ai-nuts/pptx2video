...
Pipeline position (optional post-step for pptx2video):
    Inputs:
    ...
    Steps:
        ...
        2. Resolve each logical ID through audio/manifest.json, probe its MP3,
           and load Edge word boundaries when available.
...
def find_ffmpeg_pair() -> tuple[str, str]:
    ...
    ffmpeg, ffprobe = _shared_ffmpeg_pair(component="add_subtitles")
...
def _subtitle_font_candidates(system_name: str) -> list[tuple[str, tuple[Path, ...]]]:
    """Return broad-coverage subtitle fonts in platform preference order."""
    if system_name == "Windows":
        fonts = Path(os.environ.get("WINDIR", "C:/Windows")) / "Fonts"
        return [
            ("Microsoft YaHei", (fonts / "msyh.ttc", fonts / "msyh.ttf")),
...
def resolve_subtitle_font(
    ...
    *,
    ...
    """Resolve a platform font while preserving an explicit user choice."""
    if requested and requested.strip():
        return requested.strip()
    system_name = system_name or platform.system()
    exists = path_exists or Path.is_file
    candidates = _subtitle_font_candidates(system_name)
    for family, paths in candidates:
        if any(exists(path) for path in paths):
            return family
    # libass/fontconfig may still resolve these generic platform defaults even
    # when the common font file locations are unavailable or sandboxed.
    return {
        "Windows": "Arial",
        "Darwin": "Helvetica Neue",
    }.get(system_name, "DejaVu Sans")
...
def write_ass(cues: list[Cue], path: Path, *,
    ...
    By default pptx2video appends a solid black band below the unchanged slide.
...
def ass_filter_filename(
    path: str | Path,
    *,
    system_name: str | None = None,
) -> str:
    """Return an FFmpeg-filter-safe ASS filename option.

    FFmpeg's filter parser treats a Windows drive colon as an option separator.
    Use forward slashes, escape filter metacharacters, and quote the complete
    filename value. The returned text is passed directly as one argv element,
    so it contains filter escaping only and no shell escaping.
    """
    system_name = system_name or platform.system()
    raw = str(path)
    if system_name == "Windows":
        ...
        raw = raw.replace("\\", "\\\\")
    for character in ("'", ":", ",", "[", "]", ";"):
        raw = raw.replace(character, f"\\{character}")
...
def burn_subtitles(mp4: Path, ass: Path, out: Path, ffmpeg: str, *,
    ...
    The `ass=` filter needs a filename, but its parser cannot reliably preserve
    every valid OS path character. We expose an ASCII-only relative alias and
    run FFmpeg from the alias directory instead of embedding the absolute path
    in filtergraph syntax.
    ...
    # FFmpeg's filtergraph parser applies its own quoting rules after argv has
    # already been decoded. In particular, a literal apostrophe in an absolute
    # filename cannot be made reliable merely by adding shell-style escapes.
    # Put an ASCII-only alias in the writable output directory, set that
    # directory as the child process CWD, and expose only the safe basename to
    # the filter parser.
    # The parent may still contain arbitrary Unicode, spaces, or apostrophes;
    # subprocess passes that CWD as an OS path rather than filtergraph syntax.
    with tempfile.NamedTemporaryFile(
        prefix="pptx2video_subtitles_",
        suffix=".ass",
        dir=out_abs.parent,
        delete=False,
    ) as temporary:
        ass_alias = Path(temporary.name)
    try:
        shutil.copy2(ass_abs, ass_alias)
        filters: list[str] = []
        ...
        filters.append(f"ass={ass_filter_filename(ass_alias.name)}")
        cmd = [
            ffmpeg, "-y",
            "-i", str(mp4_abs),
            "-vf", ",".join(filters),
        ...
        proc = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd=str(out_abs.parent),
        )
        if proc.returncode != 0:
            sys.exit(f"[add_subtitles] ffmpeg burn-in failed:\n{proc.stderr}")
    finally:
        ass_alias.unlink(missing_ok=True)
...
def collect_timed_inputs(
    ...
    try:
        ...
        sys.exit(f"[add_subtitles] {exc}")
    for sec in _load_script_sections(script_json):
        sid = str(sec["id"])
        try:
            mp3 = resolve_audio_path(audio_dir, sid, file_map=audio_file_map)
        except ValueError as exc:
            sys.exit(f"[add_subtitles] {exc}")
        if not mp3.is_file():
        ...
        legacy_filename = legacy_audio_filename(sid)
        note_md = (
            notes_dir / f"{Path(legacy_filename).stem}.md"
            if legacy_filename is not None
            else None
        )
        if not text.strip() and note_md is not None and note_md.is_file():
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--font", default=None,
                    help="Font for burned-in subtitles. By default, detect a broad-coverage "
                         "font installed on Windows, macOS, or Linux and use an explicit "
                         "platform fallback when common font paths are unavailable.")
...
def main() -> int:
    ...
    font_name = resolve_subtitle_font(args.font)
    subtitle_bar_height = (
    ...
    print(
        f"[add_subtitles] wrote ASS at {video_w}×{output_h} with "
        f"font {font_name!r} size {font_size} → {ass_path}"
    )
