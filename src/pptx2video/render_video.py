...
Pipeline position (rendering stage of pptx2video):
    Inputs:
...
from .browser import find_system_chromium
...
# Keep every native entrance name registered by editable_pptx.EFFECT_NAMES
# explicit here.  The strategy value is part of the render/QA contract: exact
# implementations use a literal name, while deliberately simplified effects
# carry an ``_approx`` suffix so a complex PowerPoint preset is never silently
# presented as an unrelated effect.
...
SUPPORTED_ANIMATION_NAMES = frozenset(ANIMATION_RENDER_STRATEGIES)
...
def find_chrome() -> str | None:
    ...
    return find_system_chromium(os.environ)


def find_ffmpeg_pair() -> tuple[str, str]:
    """Return the shared doctor/runtime FFmpeg selection."""
    ffmpeg, ffprobe = _shared_ffmpeg_pair(component="render_video")
    ...
    *,
    ...
    try:
        ...
        sys.exit(
            ...
            "Install the svg extra, run `python -m playwright install chromium`, "
            "and verify it with `pptx2video doctor --svg`; or rerun with "
            "--frame-source pptx for the legacy LibreOffice path."
        )
...
class SlidePair:
    ...
    def logical_id(self) -> str:
        """Return the script/manifest ID without depending on the MP3 filename."""
        return self.section_id or self.audio.stem
...
def load_animation_manifest(
    ...
    *,
    ...
    for pair, raw_slide in zip(pairs, slides):
        ...
        if slide_id != pair.logical_id:
            sys.exit(
                f"[render_video] animation slide {index} id {slide_id!r} does not "
                f"match audio/script id {pair.logical_id!r}"
            )
        effects: list[AnimationEffect] = []
...
def render_svg_animation_assets(
    ...
    *,
    ...
    try:
        ...
        sys.exit(
            "[render_video] animated SVG rendering requires Pillow, the svg extra, "
            "and Chromium. Run `python -m playwright install chromium` and "
    ...
    *,
    ...
    if script_json is not None:
        ...
        try:
            ...
            ordered = [
                resolve_audio_path(audio_dir, sid, file_map=audio_file_map)
                for sid in ids
            ]
        except ValueError as exc:
            sys.exit(f"[render_video] {exc}")
    ...
    try:
        ...
        sys.exit(f"[render_video] {exc}")
    if audio_file_map:
        ordered = [audio_dir / name for name in audio_file_map.values()]
...
def resolve_audio_section_ids(audio_dir: Path, audio_files: list[Path]) -> list[str]:
    """Resolve ordered physical MP3 paths back to their logical section IDs."""
    try:
        audio_file_map = load_audio_manifest_file_map(audio_dir)
    except ValueError as exc:
        sys.exit(f"[render_video] {exc}")
    if not audio_file_map:
        return [audio.stem for audio in audio_files]

    ids_by_filename = {
        filename: section_id
        for section_id, filename in audio_file_map.items()
    }
    section_ids: list[str] = []
    for audio in audio_files:
        section_id = ids_by_filename.get(audio.name)
        if section_id is None:
            sys.exit(
                f"[render_video] audio file {audio.name!r} has no logical ID in "
                f"{audio_dir / 'manifest.json'}"
            )
...
def pair_slides(frames: list[Path], audio_files: list[Path],
                ...
                section_ids: list[str] | None = None) -> list[SlidePair]:
    ...
    if section_ids is None:
        section_ids = [audio.stem for audio in audio_files]
    if len(section_ids) != len(audio_files):
        sys.exit(
            f"[render_video] audio/logical-ID count mismatch: "
            f"{len(audio_files)} mp3s vs {len(section_ids)} logical IDs"
        )
    pairs = []
    for i, (frame, audio, section_id) in enumerate(
        zip(frames, audio_files, section_ids),
        start=1,
    ):
...
def load_visual_cues(
    ...
    *,
    ...
    by_id = {pair.logical_id: pair for pair in pairs}
...
def _animation_strategy(name: str) -> str:
    try:
        return ANIMATION_RENDER_STRATEGIES[name]
    except KeyError as exc:
        raise ValueError(f"unsupported animation effect: {name}") from exc
...
def _animation_sample_times(
    ...
    *,
    ...
    if strategy in {"appear", "cut_instant"}:
...
def _animation_overlay_filters(
    ...
    *,
    ...
    if strategy in {"alpha_fade", "dissolve_alpha_approx"}:
    ...
    if strategy == "fly_from_left":
    ...
    elif strategy == "peek_from_left_approx":
        travel = max(72, min(360, int(round(layer.width * 0.45))))
        x_expr = f"{layer.x}-{travel}*(1-{progress})"
    ...
    elif strategy == "wheel_circle_approx":
        radius = math.hypot(layer.width / 2.0, layer.height / 2.0)
        source_filter += (
            ",geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':"
            f"a='alpha(X,Y)*lte(hypot(X-W/2,Y-H/2),{radius:.3f}*{alpha_progress})'"
        )
    elif strategy == "box_reveal":
        source_filter += (
            ",geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':"
            "a='alpha(X,Y)*lte("
        ...
        source_filter += (
            ",geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':"
            f"a='alpha(X,Y)*lte((X/max(1,W)+Y/max(1,H))/2,{alpha_progress})'"
        )
    elif strategy in {
        "stretch_horizontal",
        "expand_from_center",
        "swivel_horizontal_approx",
    }:
        if strategy == "expand_from_center":
            width_scale = height_scale = f"0.55+0.45*{progress}"
        elif strategy == "stretch_horizontal":
            width_scale = f"0.08+0.92*{progress}"
            height_scale = "1"
        else:
            width_scale = f"0.04+0.96*{progress}"
            height_scale = "1"
            source_filter += (
                f",fade=t=in:st={start:.3f}:d={min(0.16, duration):.3f}:alpha=1"
            )
        pad_margin = 2
        source_filter += (
            f",scale=w='min({layer.width},max(2,floor({layer.width}*({width_scale}))))':"
            f"h='min({layer.height},max(2,floor({layer.height}*({height_scale}))))':eval=frame,"
            f"pad=w={layer.width + pad_margin * 2}:h={layer.height + pad_margin * 2}:"
            "x='(ow-iw)/2':y='(oh-ih)/2':"
            "color=0x00000000:eval=frame"
        )
...
def concat_segments(segments: list[Path], out_path: Path, ffmpeg: str,
    ...
    with tempfile.TemporaryDirectory(
        ...
        # ffconcat has its own quoting grammar. POSIX shell quoting corrupts
        # filenames containing apostrophes, especially on Windows. Stage each
        # source behind a deterministic ASCII-only basename and make the alias
        # directory FFmpeg's CWD so the list never contains a user-controlled
        # path. Prefer hard links to avoid duplicating large segment files; the
        # temporary directory is under the output parent and therefore normally
        # shares a volume with the render bundle. Copy only when linking is not
        # supported.
        list_lines: list[str] = []
        for index, segment in enumerate(segments, start=1):
            source = segment.resolve()
            alias = Path(td) / f"segment-{index:06d}.mp4"
            try:
                os.link(source, alias)
            except OSError:
                shutil.copy2(source, alias)
            list_lines.append(f"file '{alias.name}'")
...
def verify_output(mp4: Path, ffprobe: str, ffmpeg: str) -> float:
    """Confirm the file has a video stream and return its duration."""
    if ffprobe != ffmpeg:
        out = subprocess.run(
            [ffprobe, "-v", "error", "-show_entries",
             "format=duration:stream=codec_type", "-of", "json", str(mp4)],
        ...
        if out.returncode == 0 and out.stdout.strip():
            try:
                ...
                if "video" not in stream_types:
                    sys.exit(f"[render_video] could not verify {mp4}: MP4 has no video stream.")
                if duration > 0:
                    return duration

    out = subprocess.run([ffmpeg, "-i", str(mp4)], capture_output=True, text=True)
    if not re.search(r"Stream\s+#.*:\s*Video:", out.stderr):
        sys.exit(f"[render_video] could not verify {mp4}: MP4 has no video stream.")
    m = re.search(r"Duration:\s+(\d+):(\d+):([\d.]+)", out.stderr)
    if not m:
...
def main() -> int:
    ...
    ap.add_argument("--browser-executable", default=None,
                    help="Chrome/Chromium executable for SVG frame screenshots. Defaults to "
                         "PPTX2VIDEO_CHROME/CHROME/CHROMIUM or a common system install.")
    ...
    ap.add_argument("--fps", type=int, default=DEFAULT_FPS, help="Output frame rate (default: 30)")
    ap.add_argument("--pad-tail", type=float, default=DEFAULT_PAD_TAIL,
                    help="Silence appended after each slide's narration (default: 0.3s)")
    ap.add_argument("--start-pad", type=float, default=DEFAULT_START_PAD,
    ...
    try:
        ...
        sys.exit(f"[render_video] invalid render timing: {exc}")
    ...
    section_ids = resolve_audio_section_ids(audio_dir, audio_files)
    pairs = pair_slides(
        frames,
        audio_files,
        ffprobe,
    ...
    for pair in pairs:
        ...
        print(
            f"[render_video]   segment {pair.index}/{len(pairs)}: {pair.logical_id} "
            f"[{pair.audio.name}] "
